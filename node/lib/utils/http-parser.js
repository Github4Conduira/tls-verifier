...
exports.getHttpRequestHeadersFromTranscript = exports.getCompleteHttpResponseFromTranscript = exports.makeHttpResponseParser = void 0;
 ...
 */
function getCompleteHttpResponseFromTranscript(transcript) {
    const serverResponseBlocks = [];
    let srvResStartIdx = -1;
    for (let i = 0; i < transcript.length; i++) {
        const msg = transcript[i];
        const content = Buffer.from(msg.message).toString();
        if (msg.senderType === api_1.TranscriptMessageSenderType.TRANSCRIPT_MESSAGE_SENDER_TYPE_SERVER
    ...
    for (let i = srvResStartIdx; i < transcript.length; i++) {
        const msg = transcript[i];
        if (msg.senderType === api_1.TranscriptMessageSenderType.TRANSCRIPT_MESSAGE_SENDER_TYPE_CLIENT) {
            break;
        }
        if (msg.redacted) {
            break;
...
exports.getCompleteHttpResponseFromTranscript = getCompleteHttpResponseFromTranscript;
/**
 ...
 */
function getHttpRequestHeadersFromTranscript(transcript) {
    ...
    const clientSentMsgs = transcript
        .filter(m => m.senderType === api_1.TranscriptMessageSenderType.TRANSCRIPT_MESSAGE_SENDER_TYPE_CLIENT)
        .slice(2);
...
exports.getHttpRequestHeadersFromTranscript = getHttpRequestHeadersFromTranscript;
