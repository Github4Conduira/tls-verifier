...
export function extractJSONValueIndex(json: string, jsonPath: string) {
	const pointers = JSONPath({ path: jsonPath, json: JSON.parse(json), wrap:false, resultType:'pointer' })
	if(!pointers) {
 ...
 */
function traverse(o: Expression, path: string, pointer: string): JSONIndex | null {
