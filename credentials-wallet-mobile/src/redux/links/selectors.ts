...
export const getLastLink = (state: RootState) => getLinksList(state)[getLinksList(state).length - 1]

export const getLinkById = (id: string | undefined) => (state: RootState) => getLinksList(state).find((link) => link.id === id)

export const isLinksLoading = (state: RootState) => state.links.loading === LoadingState.PENDING

export const isAllClaimsMintedInTempLink = (state: RootState) => getTempLink(state).claims.every((claim) => claim.status === ClaimStatus.MINTED)
...
export const isLinkMine = (id: string) => (state: RootState) => getLinkById(id)(state)?.userId !== ''
