...
const linksSlice = createSlice({
	...
	reducers: {
		...
		updateClaim: (state, action: PayloadAction<{ link: Link, claim: Claim }>) => ({
			...state,
			list: state.list.map((link) => link.id === action.payload.link.id
				? {
					...link,
					claims: link.claims.map((claim) => claim.internalId === action.payload.claim.internalId ? action.payload.claim : claim
					),
				}
				: link
			),
		...
		updateTempClaim: (state, action: PayloadAction<{ claim: Claim }>) => ({
			...
			tempLink: {
				...state.tempLink,
				claims: state.tempLink.claims.map((claim) => claim.id === action.payload.claim.id ? action.payload.claim : claim
				),
			},
		}),
		resetTempLink: (state) => ({
